...
export function installDiagramEditor() {
  ...
  document.addEventListener('submit', event => { if (event.target.id === 'diagram-save-form') { event.preventDefault(); saveToRepo(event.target); } });
  ...
  document.addEventListener('pointerdown', event => {
    ...
    if (node) {
      ...
      if (tool === 'edge') { if (!edgeStart) edgeStart = selectedNode; else { setSource(`${source.trim()}\n${edgeStart} -> ${selectedNode}`); edgeStart = ''; } return; }
  ...
  document.addEventListener('pointerup', event => { if (drag?.node && Math.hypot(event.clientX - drag.x, event.clientY - drag.y) > 5 && currentSvgElement()) moveNode(drag.node, scenePoint(event)); if (drag?.shape !== undefined && Math.hypot(event.clientX - drag.x, event.clientY - drag.y) > 5 && currentSvgElement()) { const point = scenePoint(event); moveCanvasShape(drag.shape, point.x - drag.point.x, point.y - drag.point.y); } drag = null; });
}
