...
let editing = false, scope = 'day', undo = [], redo = [], selectedLesson = -1;
...
function handleClick(event) {
  ...
  if (button.dataset.shift !== undefined) return mutate(next => { const lessons = currentLessons(), i = Number(button.dataset.shift), j = i + Number(button.dataset.step); if (j < 0 || j >= lessons.length) return; [lessons[i].start, lessons[j].start] = [lessons[j].start, lessons[i].start]; [lessons[i].end, lessons[j].end] = [lessons[j].end, lessons[i].end]; replaceLessons(next, lessons); });
...
export function installScheduleEditor() {
  ...
  document.addEventListener('submit', event => {
    if (event.target.id === 'schedule-lesson-form') {
      ...
      const lesson = normalizeLesson({ ...values, break: form.elements.break.checked }, selectedLesson >= 0 ? currentLessons()[selectedLesson].id : crypto.randomUUID());
